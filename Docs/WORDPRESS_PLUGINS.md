...
A separately installed WordPress plugin can extend MSPress without being placed in the internal extension directory. The bridge is the `mspress_register_plugin` action.
...
<?php
/**
 * Plugin Name: My MSPress Extension
 */

use MSPress\Includes\Plugins\Plugins;
use MSPress\Includes\Plugins\PluginInterface;

add_action('mspress_register_plugin', static function (Plugins $plugins): void {
...
final class MyPluginExtension implements PluginInterface {
    ...
    public function get_description(): string { return 'Adds a MSPress integration.'; }
...
Use `PermissionHelper` for protected routes and import `Response` from `MSPress\API` when using the host response envelope. Shortcode callbacks must return rendered output.
...
Implement settings providers for defaults and generated settings pages. Read through `MSPress\Includes\Settings\Settings` and sanitize all submitted values before storing them. External extensions own their translation catalogs and language files.
...
1. Declare MSPress as a dependency or check for its classes.
2. Load the extension class before the registration hook runs.
3. Register on `mspress_register_plugin`.
...
7. Test activation with MSPress active and inactive.
