...
```php
use MSPress\API\ResourceService;
...
Define narrowly scoped, documented hooks when another plugin needs to change a payload or observe a domain event. Use the plugin slug as the prefix, for example `mspress_resource_payload` or `mspress_resource_saved`. Apply authorization before exposing data, and document whether hooks run before or after persistence.
