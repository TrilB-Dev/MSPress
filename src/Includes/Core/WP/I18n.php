...
final class I18n {
	/**
	 * Default authoring locale for plugin translations.
	 * 
	 */
	 ...
	 */
	public function __construct( string $domain = 'mspress', ?string $languages_path = null, ?string $plugin_file = null, string $default_locale = self::DEFAULT_LOCALE ) {
		$this->domain = sanitize_key( $domain );
		$plugin_file = $plugin_file ?? ( defined( 'MSPress_FILE' ) ? MSPress_FILE : null );
		$plugin_basename = $plugin_file && function_exists( 'plugin_basename' ) ? plugin_basename( $plugin_file ) : null;
		$default_path = $plugin_basename ? dirname( $plugin_basename ) . '/Languages' : 'Languages';
