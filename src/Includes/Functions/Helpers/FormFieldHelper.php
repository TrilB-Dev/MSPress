...
final class FormFieldHelper {
    ...
    public static function button( string $label, array $options = [] ): string {
        ...
        $attributes = array_merge( $options['attributes'] ?? [], $options );
        unset( $attributes['attributes'], $attributes['href'], $attributes['class'], $attributes['type'], $attributes['label'], $attributes['disabled'] );
    ...
    public static function dropdown_button( string $label, array $items = [], array $options = [] ): string {
        ...
        if ( ! empty( $options['split'] ) ) {
            $toggle .= '<span class="visually-hidden">' . esc_html__( 'Toggle Dropdown', 'mspress' ) . '</span>';
        }
    ...
    public static function button_toolbar( array $groups = [], array $options = [] ): string {
        ...
        unset( $attributes['attributes'], $attributes['id'], $attributes['class'], $attributes['aria_label'], $attributes['gap'] );
            $attributes['id'] = (string) ( $options['id'] ?? sanitize_title( 'mspress-button-toolbar' ) );
    ...
    private static function render_bootstrap_select( string $name, array $options ): string {
        ...
        $settings = [
            ...
            'live_search_placeholder' => [ 'live-search-placeholder', __( 'Search or create', 'mspress' ) ],
            'open_options_text' => [ 'open-options-text', __( 'Create "{0}"', 'mspress' ) ],
            'selected_text_format' => [ 'selected-text-format', 'count' ],
            'selected_items_style' => [ 'selected-items-style', 'tags' ],
            'selected_tag_remove_label' => [ 'selected-tag-remove-label', __( 'Remove', 'mspress' ) ],
            'placeholder' => [ 'placeholder', __( 'Select options', 'mspress' ) ],
            'width' => [ 'width', '100%' ],
            'size' => [ 'size', null ],
    ...
    public static function label( string $for, string $text, array $options = [] ): string {
        ...
        if ( ! empty( $options['tooltip'] ) ) {
            ...
            $html .= ' <button type="button" class="btn btn-link p-0 align-baseline mspress-field-tooltip" data-bs-toggle="tooltip" data-bs-placement="top" title="' . esc_attr( (string) $options['tooltip'] ) . '" aria-label="' . esc_attr( (string) $options['tooltip'] ) . '"><i class="' . esc_attr( $icon ) . '" aria-hidden="true"></i></button>';
        }
