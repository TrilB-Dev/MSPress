<?php
/**
 * Tokenized MSPress permalink support.
 *
 * @package MSPress
 */
...
final class PermalinkHelper {
    public const OVERRIDE_META = '_mspress_permalink';

    public static function token_definitions(): array {
        return [
            '%root%' => __( 'The root MSPress slug.', 'mspress' ),
            '%root_category%' => __( 'The Wiki categories, from parent to child.', 'mspress' ),
            '%root_tags%' => __( 'The tags assigned to the Wiki container.', 'mspress' ),
            '%wiki%' => __( 'The Wiki slug.', 'mspress' ),
            '%wiki_category%' => __( 'The Wiki page categories, from parent to child.', 'mspress' ),
            '%wiki_tag%' => __( 'The tags assigned to the Wiki page.', 'mspress' ),
            '%wiki_page%' => __( 'The Wiki page slug.', 'mspress' ),
        ];
    }
    ...
    public static function page_url( \WP_Post $page ): string {
        $wiki_id = absint( get_post_meta( $page->ID, '_mspress_wiki_id', true ) );
    ...
    public static function rewrite_rule(): void {
        add_rewrite_rule( '^(.+?)/?$', 'index.php?mspress_path=$matches[1]', 'top' );
    ...
    public static function resolve_request( array $vars ): array {
        $requested_path = isset( $vars['mspress_path'] ) ? trim( urldecode( (string) $vars['mspress_path'] ), '/' ) : '';
        if ( '' === $requested_path ) {
            return $vars;
        }
