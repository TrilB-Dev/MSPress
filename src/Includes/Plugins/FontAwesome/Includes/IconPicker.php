...
class IconPicker {
    ...
    private function init() {
        ...
        if ( function_exists( 'add_action' ) ) {
            $this->loader->register_component( $this, [
                [ 'type' => 'action', 'hook' => 'wp_ajax_mspress_fontawesome_search_icons', 'callback' => 'ajax_search_icons' ],
                [ 'type' => 'action', 'hook' => 'wp_ajax_mspress_fontawesome_get_icon_data', 'callback' => 'ajax_get_icon_data' ],
            ] )->run();
        }
    }
    ...
    public function ajax_search_icons() {
        if ( ! AjaxHelper::has_valid_nonce( 'mspress_fontawesome_picker' ) ) {
    ...
    public function search_icons( $search = '', $pack = 'classic', $style = 'solid', $page = 1, $per_page = 50 ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Error searching icons: ' . $e->getMessage() );
    ...
    public function get_icon_data( $icon_name, $pack = 'classic', $style = 'solid' ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Error getting icon data: ' . $e->getMessage() );
            return null;
        }
    }
    ...
    private function get_available_icons_from_api( $pack, $style ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Error getting icons from API: ' . $e->getMessage() );
            return array();
        }
    }
    ...
    private function query_fontawesome_api( $pack, $style ) {
        ...
        try {
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Executing GraphQL query for pack=' . $pack . ', style=' . $style );
            ...
            if ( json_last_error() !== JSON_ERROR_NONE ) {
                LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: JSON decode error: ' . json_last_error_msg() );
                return array();
            }

            if ( ! isset( $response['data']['release']['icons'] ) ) {
                LoggerHelper::write_log( [
                    'message' => 'MSPress FontAwesome IconPicker: Invalid API response structure.',
                    'response' => $response,
                ] );
                return array();
            }
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Retrieved ' . count( $all_icons ) . ' total icons from API' );
            ...
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Filtered to ' . count( $icons ) . ' icons for pack=' . $pack . ', style=' . $style );
            return $icons;

        } catch ( Exception $e ) {
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Error querying FontAwesome API: ' . $e->getMessage() );
            LoggerHelper::write_log( 'MSPress FontAwesome IconPicker: Exception trace: ' . $e->getTraceAsString() );
            return array();
        }
    }
    ...
    public function render_picker( $input_name, $selected_icon = '', $args = array() ) {
        $args = wp_parse_args( $args, array(
            'id' => uniqid( 'fa-picker-' ),
            'input_id' => '',
            'class' => 'mspress-fa-picker',
            'button_text' => __( 'Choose Icon', 'mspress' ),
            'preview_size' => '24px',
        ) );
        ...
        ob_start();
        ?>
        <div class="mspress-fa-picker-container" id="<?php echo esc_attr( $args['id'] ); ?>">
                 <?php echo wp_kses_post( FormFieldHelper::input( $input_name, $selected_icon, [ 'type' => 'hidden', 'id' => $args['input_id'], 'class' => 'mspress-fa-picker-input' ] ) ); ?>

                <button type="button"
                    class="btn btn-secondary mspress-fa-picker-button"
                    data-picker-id="<?php echo esc_attr( $args['id'] ); ?>">
                <span class="mspress-fa-picker-preview">
                    <?php if ( $selected_data ) : ?>
                        <i class="<?php echo esc_attr( $selected_data['class'] ); ?>"
                           style="font-size: <?php echo esc_attr( $args['preview_size'] ); ?>"></i>
                    <?php endif; ?>
                </span>
                <span class="mspress-fa-picker-text">
                    <?php echo esc_html( $args['button_text'] ); ?>
                </span>
            </button>

            <div class="mspress-fa-picker-modal" style="display: none;">
                <div class="mspress-fa-picker-overlay"></div>
                <div class="mspress-fa-picker-dialog">
                    <div class="mspress-fa-picker-header">
                        <h3><?php _e( 'Choose an Icon', 'mspress' ); ?></h3>
                        <button type="button" class="mspress-fa-picker-close">&times;</button>
                    </div>

                    <div class="mspress-fa-picker-search">
                        <?php echo wp_kses_post( FormFieldHelper::input( 'mspress_fa_search', '', [ 'type' => 'search', 'class' => 'mspress-fa-picker-search-input', 'placeholder' => __( 'Search icons...', 'mspress' ) ] ) ); ?>
                        <?php echo wp_kses_post( FormFieldHelper::select( 'mspress_fa_pack', [ 'classic' => __( 'Classic', 'mspress' ), 'duotone' => __( 'Duotone', 'mspress' ), 'sharp' => __( 'Sharp', 'mspress' ), 'sharp-duotone' => __( 'Sharp Duotone', 'mspress' ), 'brands' => __( 'Brands', 'mspress' ) ], 'classic', [ 'class' => 'mspress-fa-picker-pack-filter' ] ) ); ?>
                        <?php echo wp_kses_post( FormFieldHelper::select( 'mspress_fa_style', [ 'solid' => __( 'Solid', 'mspress' ), 'regular' => __( 'Regular', 'mspress' ), 'light' => __( 'Light', 'mspress' ), 'thin' => __( 'Thin', 'mspress' ) ], 'solid', [ 'class' => 'mspress-fa-picker-style-filter' ] ) ); ?>
                    </div>

                    <div class="mspress-fa-picker-results">
                        <div class="mspress-fa-picker-loading">
                            <?php _e( 'Loading icons...', 'mspress' ); ?>
                        </div>
                    </div>

                    <div class="mspress-fa-picker-footer">
                        <button type="button" class="btn btn-secondary mspress-fa-picker-cancel">
                            <?php _e( 'Cancel', 'mspress' ); ?>
                        </button>
                        <button type="button" class="btn btn-primary mspress-fa-picker-select" disabled>
                            <?php _e( 'Select Icon', 'mspress' ); ?>
                        </button>
                    </div>
                </div>
            </div>
