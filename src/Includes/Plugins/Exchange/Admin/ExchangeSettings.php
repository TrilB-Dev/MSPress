...
final class ExchangeSettings {
    public function render(): void {
        ...
        echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" class="mspress-exchange-form">';
        echo '<input type="hidden" name="action" value="mspress_exchange_save_settings">';
        wp_nonce_field( 'mspress_exchange_save_settings' );
        echo '<h2>' . esc_html__( 'Sending', 'mspress' ) . '</h2>';
        ...
        echo '<p><label for="mspress-exchange-default-sender">' . esc_html__( 'Default sender', 'mspress' ) . '</label><select class="selectpicker form-select" data-live-search="true" id="mspress-exchange-default-sender" name="settings[default_sender]"><option value="">' . esc_html__( 'Choose a sender', 'mspress' ) . '</option>';
        ...
        echo '</select></p>';
        echo '<h2>' . esc_html__( 'Sender profiles', 'mspress' ) . '</h2><p><button type="button" class="button" data-exchange-import>' . esc_html__( 'Import directory mailboxes', 'mspress' ) . '</button></p>';
        ...
        echo '</tbody></table></div><p><button type="submit" class="button button-primary">' . esc_html__( 'Save Exchange settings', 'mspress' ) . '</button></p></form>';
    ...
    private function render_templates(): void {
        echo '<h2>' . esc_html__( 'Email templates', 'mspress' ) . '</h2>';
    ...
    private function render_logs( array $logs ): void {
        echo '<h2>' . esc_html__( 'Sent email log', 'mspress' ) . '</h2><div class="table-responsive"><table class="widefat striped"><thead><tr><th>' . esc_html__( 'Date', 'mspress' ) . '</th><th>' . esc_html__( 'Sender', 'mspress' ) . '</th><th>' . esc_html__( 'Recipients', 'mspress' ) . '</th><th>' . esc_html__( 'Subject', 'mspress' ) . '</th></tr></thead><tbody>';
