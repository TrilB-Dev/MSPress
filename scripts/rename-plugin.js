...
async function collectAnswers() {
  ...
  try {
    ...
    const slug = await ask(rl, 'Plugin slug (lowercase kebab-case)', toSlug(displayName));
    ...
    const packageVendor = await ask(rl, 'Composer package vendor', slug.replaceAll('-', ''));
...
function buildReplacements(values) {
  return new Map([
    ['author/plugin-name', values.packageName],
    ['pluginname/plugin-name', values.packageName],
    ['https://example.com/plugin-name', values.pluginUri],
    ['Text Domain: plugin-name', `Text Domain: ${values.textDomain}`],
    ['X-Domain: plugin-name', `X-Domain: ${values.textDomain}`],
    ["'plugin-name'", `'${values.textDomain}'`],
    ['"plugin-name"', `"${values.textDomain}"`],
    ['https://example.com', values.authorUri],
    ['Plugin Name', values.displayName],
    ['plugin_name_status', values.shortcode],
    ['plugin_name', values.shortcodePrefix],
    ['plugin-name.js', `${values.assetBasename}.js`],
    ['plugin-name.css', `${values.assetBasename}.css`],
    ['pluginname.php', values.bootstrapFilename],
    ['your-plugin-name', values.slug],
    ['PluginName', values.className],
    ['PLUGINNAME', values.namespace],
    ['YourName', values.author],
    ['plugin_name_status', values.shortcode],
    ['plugin_name', values.shortcodePrefix],
    ['plugin-name', values.slug],
...
async function main() {
  if (process.argv.includes('--help') || process.argv.includes('-h')) {
    ...
    console.log('');
    console.log('Guides you through renaming the MSPress plugin template.');
  ...
  console.log('MSPress plugin rename assistant');
